...
atext = 'The split method is used to split the strings and store them in the list.'+\
        ...
        'If a delimiter is not specified or is None, a different splitting algorithm is applied: runs of consecutive whitespace are regarded as a single separator,'+ \
...
def Problem2(atext):
    ltext = atext.split(" ")
    anstext = reduce(lambda x, y: x+' '+y,
                    map(lambda x: 'xxx' if x.lower() == 'the' else x, ltext))
    return anstext
